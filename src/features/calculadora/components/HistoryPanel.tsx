...
export function HistoryPanel({
  ...
  return (
    <section className="rounded-2xl border border-border bg-card-bg/60 p-5 backdrop-blur-xl sm:p-6">
      <div className="mb-4 flex items-center gap-2.5">
        ...
        {!loading && (
          <span className="rounded-full border border-border bg-foreground/5 px-2 py-0.5 text-[10px] font-medium text-text-secondary">
            {calculations.length}
          </span>
        )}
      </div>
          ...
          <AnimatePresence initial={false}>
            {calculations.length === 0 && (
              <motion.p
                ...
                className="rounded-2xl border border-dashed border-border bg-foreground/[0.03] px-4 py-8 text-center text-sm text-text-secondary"
              >
